...
TEST(DDGIAtlasTests, PacksEachAtlasAgainstItsOwnTileExtent) {
  auto irradiance = nuri::packDDGIAtlas(
      2048u, nuri::kDDGIIrradianceTileExtent, glm::uvec2(16'384u));
  auto distance = nuri::packDDGIAtlas(
      2048u, nuri::kDDGIDistanceTileExtent, glm::uvec2(16'384u));
...
TEST(DDGIAtlasTests, RejectsPackingThatExceedsDeviceDimensions) {
  auto result = nuri::packDDGIAtlas(
      65'536u, nuri::kDDGIDistanceTileExtent, glm::uvec2(512u));
...
TEST(DDGIAtlasTests, EstimatesAllocatedTextureAndStateBytesExactly) {
  ...
  EXPECT_EQ(estimate->persistentBytes,
            estimate->irradianceBytes + estimate->distanceBytes +
...
TEST(DDGIAtlasTests, CanonicalIndexAndRingMappingRoundTrip) {
  ...
  for (uint32_t index = 0u; index < 24u; ++index) {
    const glm::uvec3 coordinate =
        nuri::ddgiProbeCoordinate(index, counts);
...
TEST(DDGIAtlasTests, CameraCellUsesMathematicalFloorAcrossZero) {
  EXPECT_EQ(nuri::ddgiCameraCell(glm::vec3(-0.01f, 1.99f, -2.0f),
                                 glm::vec3(1.0f)),
            glm::ivec3(-1, 1, -2));
}
...
TEST(DDGIAtlasTests, ScrollAdvancesRingAndInvalidatesOnlyExposedPlanes) {
  ...
  EXPECT_TRUE(nuri::isDDGINewlyExposedCoordinate(
      glm::uvec3(3u, 1u, 0u), plan, counts));
  EXPECT_TRUE(nuri::isDDGINewlyExposedCoordinate(
      glm::uvec3(1u, 0u, 1u), plan, counts));
  EXPECT_FALSE(nuri::isDDGINewlyExposedCoordinate(
      glm::uvec3(1u, 1u, 1u), plan, counts));
}

TEST(DDGIAtlasTests, FullAxisScrollInvalidatesEveryCoordinate) {
  constexpr glm::uvec3 counts(4u, 3u, 2u);
  const nuri::DDGIScrollPlan plan = nuri::makeDDGIScrollPlan(
      glm::ivec3(5, 4, 3), glm::uvec3(2u, 1u, 0u),
      glm::ivec3(5, 7, 3), counts);
...
TEST(DDGIAtlasTests, LayoutUsesCountMinusOneExtentsAndRejectsScale) {
  ...
  auto atlas = nuri::packDDGIAtlas(
      24u, nuri::kDDGIIrradianceTileExtent, glm::uvec2(1024u));
  ...
  auto layout = nuri::makeDDGIVolumeLayout(
      nuri::makeDDGIVolumeId(1u, 1u), desc, rigid, *atlas, *atlas, 7u);
  ...
  auto invalid = nuri::makeDDGIVolumeLayout(
      nuri::makeDDGIVolumeId(1u, 1u), desc, scaled, *atlas, *atlas, 8u);
