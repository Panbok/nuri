...
void writeMeshletVertex(uint outputIndex, uint vertexIndex, InstanceData inst,
                        ...
                        uint meshletIndex, uint selectedLod) {
  ...
  const vec3 normal =
      decodePackedNormalFrom(batch.vertexBuffer, packedVertexFormat,
                             vertexIndex);
  const vec4 tangent =
      decodePackedTangentFrom(batch.vertexBuffer, packedVertexFormat,
                              vertexIndex);
  const vec2 uv0 =
...
void main() {
  ...
  const uint batchIndex = meshletPayload.batchIndex;
  ...
  const uint selectedLod = meshletPayload.selectedLod;
