...
bool meshletOcclusionCull(MeshletDescriptorGpuData meshlet, InstanceData inst,
                          uint meshletFlags) {
  const uint depthPyramidFlags = pc.frameData.sceneDepthPyramidInfo.w;
  const bool previousFrameDepth =
      (depthPyramidFlags & kFrameDataDepthPyramidPreviousFrame) != 0u;
  const bool currentFrameDepth =
  ...
  const mat4 depthViewProj = currentFrameDepth
                                 ? pc.frameData.proj * pc.frameData.view
                                 : pc.frameData.previousViewProj;
  ...
  for (uint cornerIndex = 0u; cornerIndex < 8u; ++cornerIndex) {
    ...
    vec4 clip = depthViewProj * vec4(corner, 1.0);
    if (clip.w <= 1.0e-5) {
      return false;
    }
    ...
    vec2 uv = clipNdcToDepthUv(ndc.xy);
    uvMin = min(uvMin, uv);
    uvMax = max(uvMax, uv);
...
void main() {
  ...
  if (candidateSpan != 0u && instanceCount != 0u) {
    ...
    if (instanceLocal < instanceCount) {
      ...
      const uint selectedLod = selectMeshletLod(lodRange, globalInstanceId,
                                                meshletFlags, batch.flags.y);
