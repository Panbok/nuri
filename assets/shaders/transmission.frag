...
vec3 sampleTransmissionColor(vec2 uv, float roughness, float ior) {
  ...
  vec3 level0 =
      textureBindless2D(getSceneColorPyramidTexId(pc.frameData, 0u),
                        pc.frameData.sceneColorSamplerId, clampedUv).rgb;
  vec3 level1 = level0;
  vec3 level2 = level0;
  ...
  if (level1TexId != kInvalidTextureBindlessIndex) {
    level1 = textureBindless2D(level1TexId,
                               pc.frameData.sceneColorSamplerId,
                               clampedUv).rgb;
  }
...
vec3 getIndirectTransmission(vec3 n, vec3 v, float roughness, vec3 baseColor,
                             vec3 f0, vec3 f90, vec3 worldPos,
                             mat4 modelMatrix, float ior, float thickness,
                             vec3 attenuationColor, float attenuationDistance) {
...
void main() {
  ...
  if (transmissionTexId != kInvalidTextureBindlessIndex) {
    transmissionFactor *=
        textureBindless2D(transmissionTexId, matSampler,
                          uvTransmission).r;
  }
  ...
  if (thicknessTexId != kInvalidTextureBindlessIndex) {
    thickness *=
        textureBindless2D(thicknessTexId, matSampler, uvThickness).g;
  }
  ...
  vec3 indirectDiffuseTerm = mix(ibl.iblDiffuse, indirectTransmission,
                                 transmissionFactor);
  vec3 directDiffuseTerm   = mix(directDiffuse, directTransmission,
                                 transmissionFactor);
