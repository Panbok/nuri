...
const vec2 kCorners[6] =
    vec2[6](vec2(-1.0, -1.0), vec2(1.0, -1.0), vec2(1.0, 1.0), vec2(1.0, 1.0),
            vec2(-1.0, 1.0), vec2(-1.0, -1.0));

vec3 probeNominalLocalPosition(DDGIVolumeGpuData volume, uvec3 coordinate) {
  const ivec3 cameraCell =
      ivec3(volume.generations.x, volume.generations.y, volume.generations.w);
...
vec4 stateColor(uint state, float relocation, float age, uint debugView) {
  if (debugView == 10u) {
    ...
    return vec4(mix(vec3(0.1, 0.7, 1.0), vec3(1.0, 0.18, 0.05), amount), 0.9);
  }
  if (debugView == 11u) {
    return vec4(mix(vec3(0.1, 1.0, 0.2), vec3(1.0, 0.12, 0.05), age), 0.9);
  }
  if (state == kDDGIProbeStateUninitialized)
    return vec4(1.0, 0.15, 1.0, 0.9);
  if (state == kDDGIProbeStateOff)
    return vec4(1.0, 0.08, 0.05, 0.75);
  if (state == kDDGIProbeStateSleeping)
    return vec4(0.25, 0.3, 0.35, 0.65);
  if (state == kDDGIProbeStateNewlyAwake)
    return vec4(1.0, 0.72, 0.12, 0.9);
  if (state == kDDGIProbeStateAwake)
    return vec4(1.0, 0.95, 0.18, 0.9);
  if (state == kDDGIProbeStateNewlyVigilant)
    return vec4(0.2, 1.0, 0.85, 0.9);
...
void main() {
  ...
  const uvec3 physical =
      uvec3(probe % counts.x, (probe % xy) / counts.x, probe / xy);
  const uvec3 logical =
  ...
  const vec3 billboardPosition =
      worldPosition + (pc.cameraRightAndScale.xyz * corner.x +
                       pc.cameraUpAndDebugView.xyz * corner.y) *
                          scale;
  outLocal = corner;
  ...
  const float relocation =
      length(state.relocation.xyz) / max(0.5 * minSpacing, 1.0e-6);
  const float age =
      clamp(float(pc.submittedSequence -
                  min(pc.submittedSequence, state.stateAgeFlags.y)) /
                64.0,
            0.0, 1.0);
