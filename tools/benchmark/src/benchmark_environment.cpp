...
BenchmarkEnvironment collectBenchmarkEnvironment(
  ...
  env.repoRoot = benchmarkRepoRoot();
  ...
  env.cmakeToolProfile = NURI_BENCHMARK_TOOL_PROFILE;
  env.vcpkgManifestFeatures = NURI_BENCHMARK_VCPKG_MANIFEST_FEATURES;
  env.buildType = NURI_BENCHMARK_BUILD_TYPE;
  env.buildShared = macroBool(NURI_BENCHMARK_BUILD_SHARED);
  env.loggingEnabled = macroBool(NURI_BENCHMARK_WITH_LOGGING);
  env.assertsEnabled = macroBool(NURI_BENCHMARK_WITH_ASSERTS);
  env.tracyEnabled = macroBool(NURI_BENCHMARK_WITH_TRACY);
  env.tracyGpuEnabled = macroBool(NURI_BENCHMARK_WITH_TRACY_GPU);
  env.tracyGpuDrawZonesEnabled =
      macroBool(NURI_BENCHMARK_WITH_TRACY_GPU_DRAW_ZONES);
  env.tracyDiagnostic = tracyDiagnostic;
  env.devChecks = macroBool(NURI_BENCHMARK_DEV_CHECKS);
  return env;
}
