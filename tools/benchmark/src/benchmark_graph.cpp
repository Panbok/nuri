...
writeBenchmarkGraphHtml(std::span<const BenchmarkReport> reports,
  ...
  out << "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">"
      ...
      << "</title><style>"
         ":root{color-scheme:dark;--bg:#101318;--panel:#181d25;--panel2:#"
         "202734;"
         ...
         "*{box-sizing:border-box}body{margin:0;background:linear-gradient("
         "180deg,#101318,#151a22);"
         ...
         "header{margin-bottom:22px}h1{font-size:32px;line-height:1.1;margin:0 "
         "0 8px}"
         "h2{font-size:16px;margin:0}.subtle{color:var(--muted);max-width:"
         "760px}"
         ".summary{display:grid;grid-template-columns:repeat(auto-fit,minmax("
         "170px,1fr));gap:12px;margin:20px 0}"
         ".stat{background:var(--panel);border:1px solid "
         "var(--grid);border-radius:8px;padding:14px}"
         ".stat "
         "b{display:block;font-size:24px}.panel,.metric-card{background:rgba("
         "24,29,37,.94);"
         "border:1px solid var(--grid);border-radius:8px;padding:18px;"
         "box-shadow:0 12px 36px "
         "rgba(0,0,0,.22)}.table-wrap{overflow:auto;margin-top:12px}"
         ".panel{margin:16px 0}.metric-section{margin:26px 0 "
         "0}.section-head{display:flex;"
         "align-items:flex-end;justify-content:space-between;gap:18px;margin:0 "
         "0 12px}"
         ".section-head h2{font-size:20px}.section-head p{margin:5px 0 "
         "0;color:var(--muted)}"
         ".section-head span{color:var(--muted);border:1px solid "
         "var(--grid);border-radius:999px;padding:4px 10px}"
         ".metric-grid{display:grid;grid-template-columns:repeat(auto-fit,"
         "minmax(420px,1fr));gap:14px}"
         "table{width:100%;border-collapse:collapse;min-width:680px}th,td{text-"
         "align:left;"
         "border-bottom:1px solid var(--grid);padding:9px "
         "10px}th{color:var(--muted);font-weight:600}"
         ".metric-head{display:flex;align-items:center;justify-content:space-"
         "between;gap:16px;margin-bottom:14px}"
         ".metric-head "
         "span{color:#0b1117;background:var(--accent);border-radius:999px;"
         "padding:3px 10px;font-weight:700}"
         ".bars{display:grid;gap:10px}.bar-row{display:grid;grid-template-"
         "columns:minmax(190px,1.4fr) minmax(160px,4fr) 112px;"
         "gap:12px;align-items:center}.bar-label{color:var(--text);overflow:"
         "hidden;text-overflow:ellipsis;white-space:nowrap}"
         ".bar-track{height:22px;border-radius:6px;background:var(--panel2);"
         "border:1px solid var(--grid);overflow:hidden}"
         ".bar-fill{height:100%;border-radius:5px;background:linear-gradient("
         "90deg,var(--accent),var(--accent2));"
         "box-shadow:0 0 18px "
         "rgba(82,209,168,.35)}.bar-value{font-variant-numeric:tabular-nums;"
         "color:var(--muted);text-align:right}"
         ".cat-cpu "
         ".bar-fill{background:linear-gradient(90deg,var(--amber),var(--warn))}"
         ".cat-gpu "
         ".bar-fill{background:linear-gradient(90deg,var(--accent),var(--cyan))"
         "}"
         ".cat-memory "
         ".bar-fill{background:linear-gradient(90deg,var(--rose),var(--amber))}"
         ".cat-pmr "
         ".bar-fill{background:linear-gradient(90deg,var(--violet),var(--"
         "accent2))}"
         ".cat-gpu-memory "
         ".bar-fill{background:linear-gradient(90deg,var(--cyan),var(--accent2)"
         ")}"
         ".cat-renderer "
         ".bar-fill{background:linear-gradient(90deg,var(--accent2),var(--"
         "violet))}"
         ".empty{color:var(--muted);margin:12px 0 "
         "0}@media(max-width:720px){.bar-row,.section-head{grid-template-"
         "columns:1fr;"
         "display:grid;gap:6px}.metric-grid{grid-template-columns:1fr}.bar-"
         "value{text-align:left}h1{font-size:26px}}"
      ...
      << "</h1><p class=\"subtle\">Self-contained benchmark graph generated "
         ...
         "use ms, memory uses MiB, and renderer counters are "
         "unitless.</p></header>"
