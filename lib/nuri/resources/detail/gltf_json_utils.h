...
/// primitive; otherwise it stores `kInvalidMaterialIndex`.
/// `primitiveMaterialIndices` stores material indices in mesh order, then
/// primitive order within each mesh, and may contain more entries than
/// `meshCount`. All material indices refer to `[0, materialCount)`, with
/// `kInvalidMaterialIndex` used when a primitive omits `primitive.material` or
...
struct GltfPrimitiveMaterialMapping {
  ...
  /// material indices or `kInvalidMaterialIndex` for
  /// multi-primitive/no-material meshes.
  ...
  /// meshes use this vector for primitive-level material assignment, with
  /// `kInvalidMaterialIndex` representing omitted or unreadable materials.
...
/// absent data represented by zero counts or `kInvalidMaterialIndex` sentinels.
/// It
