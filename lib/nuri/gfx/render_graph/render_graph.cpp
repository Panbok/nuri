...
RenderGraphBuilder::OwnedPassPayload
RenderGraphBuilder::clonePassPayload(
    const RenderGraphGraphicsPassDesc &desc) const {
  OwnedPassPayload ownedPayload(memory_);
  ownedPayload.debugLabel.assign(desc.debugLabel.data(), desc.debugLabel.size());
  ...
  for (size_t i = 0; i < desc.preDispatches.size(); ++i) {
    ...
    dispatch.debugLabel = std::string_view(
        ownedPayload.preDispatchDebugLabels[i].data(),
  ...
  for (const DrawItem &sourceDraw : desc.draws) {
    ...
    ownedPayload.drawPushConstants.push_back(std::pmr::vector<std::byte>(memory_));
  ...
  for (size_t i = 0; i < desc.draws.size(); ++i) {
    ...
    draw.pushConstants = std::span<const std::byte>(
        ownedPayload.drawPushConstants[i].data(),
...
Result<bool, std::string> RenderGraphBuilder::bindImplicitPassResources(
  ...
  for (size_t dispatchIndex = 0; dispatchIndex < desc.preDispatches.size();
    ...
    for (size_t dependencyIndex = 0;
         ...
         ++dependencyIndex) {
      const BufferHandle dependency = dispatch.dependencyBuffers[dependencyIndex];
      if (!nuri::isValid(dependency)) {
        continue;
      }
  ...
  for (size_t drawIndex = 0; drawIndex < desc.draws.size(); ++drawIndex) {
    ...
    for (const auto &[buffer, target] : bindings) {
      ...
      auto bindResult = bindDrawBuffer(pass, static_cast<uint32_t>(drawIndex),
                                       target, importResult.value(),
...
RenderGraphBuilder::addPassRecord(RenderPass pass,
  ...
  pass.dependencyBuffers = std::span<const BufferHandle>(
      storedPayload.dependencyBuffers.data(), storedPayload.dependencyBuffers.size());
  ...
  passPreDispatchBindingCounts_.push_back(static_cast<uint32_t>(preDispatchCount));
      ...
      if (texture.imported) {
        if (!nuri::isValid(texture.importedHandle)) {
          return Result<RenderGraphCompileResult, std::string>::makeError(
              "RenderGraphBuilder::compile: imported texture handle is "
              "invalid");
        }
  ...
  std::pmr::vector<uint32_t> order(memory_);
  {
    ...
    const auto addDependencyEdge = [&allDependencies, &dependencyEdgeKeys](
      ...
      allDependencies.push_back(
          DependencyEdge{.before = before, .after = after});
    };
    ...
    if (!frameOutputTextureIndices_.empty() || !sideEffectPassMarks_.empty()) {
      ...
      const auto pushRoot = [&activePassMask, &stack,
                             &compiled](uint32_t passIndex) {
    ...
    for (const DependencyEdge edge : allDependencies) {
      if (activePassMask[edge.before] == 0u ||
          activePassMask[edge.after] == 0u) {
        continue;
      }
...

  {
    ...
    for (const uint32_t passIndex : order) {
      ...
      for (uint32_t dispatchIndex = 0; dispatchIndex < preDispatchCount;
        ...
        for (uint32_t depIndex = 0; depIndex < depCount; ++depIndex) {
          ...
          if (preDispatchDependencyBindingResourceIndices_[depOffset +
                                                           depIndex] ==
              UINT32_MAX) {
            return Result<RenderGraphCompileResult, std::string>::makeError(
    ...
    for (const uint32_t passIndex : order) {
      ...
      if (dependencyCount > 0u) {
        ...
        resolvedPass.dependencyBuffers = std::span<const BufferHandle>(
            compiled.resolvedDependencyBuffers.data() +
                resolvedDependencyOffset,
            dependencyCount);
      } else {
        resolvedPass.dependencyBuffers = {};
      }

      const uint32_t passPreDispatchCount =
          passPreDispatchBindingCounts_[passIndex];
      ...
      for (uint32_t dispatchIndex = 0; dispatchIndex < passPreDispatchCount;
        ...
        if (sourceDispatch.dependencyBuffers.size() !=
        ...
        const uint32_t resolvedDispatchDependencyOffset = static_cast<uint32_t>(
        ...
        for (uint32_t depIndex = 0; depIndex < dispatchDependencyCount;
          ...
          compiled.unresolvedPreDispatchDependencyBufferBindings.push_back(
              RenderGraphCompileResult::
                  UnresolvedPreDispatchDependencyBufferBinding{
                      .orderedPassIndex = orderedPassIndex,
...

  {
    ...
    for (const PassResourceAccess &access : passResourceAccesses_) {
      if (access.passIndex >= passCount ||
          activePassMask[access.passIndex] == 0u) {
        continue;
      }
...
void RenderGraphExecutor::collectRetiredResources(
    GPUDevice &gpu, uint64_t completedFrameIndex) {
...
RenderGraphExecutor::execute(GPUDevice &gpu,
...

  {
    ...
    bool hasRetireableResources = false;
    for (const PendingFrameResources &pending : pendingFrames_) {
      if (pending.retireAfterFrame <= compiled.frameIndex) {
        hasRetireableResources = true;
        break;
      }
    }
    if (hasRetireableResources) {
      gpu.waitIdle();
...

  {
    ...
    const uint64_t retireLag =
        static_cast<uint64_t>(std::max(1u, gpu.getSwapchainImageCount())) +
        1ull;
