...
if [[ ${no_build} -eq 0 ]]; then
  "${script_dir}/_nuri_build.sh" "${mode}" bench "${build_args[@]}"
fi

build_dir="${repo_root}/build/${mode}-bench"
tool="${build_dir}/nuri-bench"
if [[ ! -x "${tool}" ]]; then
...
if [[ ${#tool_args[@]} -eq 0 ]]; then
  ...
  list|explain|run|check|compare|summarize|graph|baseline)
      ;;
    *)
