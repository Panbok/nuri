...
if [[ ${no_build} -eq 0 ]]; then
  "${script_dir}/_nuri_build.sh" "${mode}" autotest "${build_args[@]}"
fi

build_dir="${repo_root}/build/${mode}-autotest"
tool="${build_dir}/nuri-autotest"
if [[ ! -x "${tool}" ]]; then
...
if [[ ${#tool_args[@]} -eq 0 ]]; then
  ...
  case "${first_tool_arg}" in
    list|explain|run|record|approve|baseline)
      ;;
    *)
